...
export default function Index() {
  ...
  return (
    <>
      {/* SEO Meta */}
      <title>Anti Climax - Rewrite Movie Endings | Alternate Climaxes</title>
      <meta
        name="description"
      ...
      <section className="relative min-h-[85vh] flex items-center justify-center overflow-hidden">
        ...
        <div className="container relative z-10 text-center px-4">
          <h1 className="text-4xl md:text-6xl lg:text-7xl font-serif font-bold leading-tight animate-fade-in-up">
            <span className="text-gradient">Rewrite</span> the Endings
            <br />
            <span className="text-foreground/80">Hollywood Got Wrong</span>
          </h1>

          <p className="mt-6 text-lg md:text-xl text-muted-foreground max-w-2xl mx-auto animate-fade-in-up stagger-1">
            Share your alternate climaxes for movies and TV shows. 
            Join a community of cinephiles who believe great stories deserve better endings.
          </p>
          ...
          {/* Stats */}
          <div className="mt-16 flex items-center justify-center gap-12 animate-fade-in-up stagger-3">
            <div className="text-center">
              <p className="text-3xl font-serif font-bold text-gradient">1000+</p>
              <p className="text-sm text-muted-foreground mt-1">Movies</p>
            </div>
            <div className="w-px h-10 bg-border/50" />
            <div className="text-center">
              <p className="text-3xl font-serif font-bold text-gradient">User</p>
              <p className="text-sm text-muted-foreground mt-1">Climaxes</p>
            </div>
            <div className="w-px h-10 bg-border/50" />
            <div className="text-center">
              <p className="text-3xl font-serif font-bold text-gradient">Free</p>
      ...
      <section className="container py-16">
        <div className="flex items-end justify-between mb-8">
          <div>
            <h2 className="text-2xl md:text-3xl font-serif font-bold">Featured Movies</h2>
            <p className="mt-2 text-muted-foreground">Popular films waiting for your alternate ending</p>
          </div>
          <Link to="/movies" className="hidden sm:flex items-center gap-2 text-sm font-medium text-primary hover:text-primary/80 transition-colors">
            View All
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
            {featuredMovies.map((movie, index) => (
              <div key={movie.id} className={`animate-fade-in-up stagger-${index % 4 + 1}`}>
                <MovieCard 
                  movie={movie} 
                  climaxCount={climaxCounts[`${movie.id}-${movie.title.toLowerCase().replace(/[^a-z0-9\s-]/g, "").replace(/\s+/g, "-")}`] || 0}
                />
              </div>
            ))}
          </div>
        ...
        <Link to="/movies" className="sm:hidden flex items-center justify-center gap-2 mt-8 text-sm font-medium text-primary">
          View All Movies
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
          </svg>
      ...
      <section className="container py-16 border-t border-border/50">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          <div className="glass-card p-6 text-center animate-fade-in-up stagger-1">
            ...
            <p className="text-sm text-muted-foreground">Browse thousands of movies and find one with an ending that didn't satisfy you.</p>
          </div>
          ...
          <div className="glass-card p-6 text-center animate-fade-in-up stagger-2">
            ...
            <p className="text-sm text-muted-foreground">Craft your own alternate ending. Be creative—fix what Hollywood got wrong.</p>
          </div>
          ...
          <div className="glass-card p-6 text-center animate-fade-in-up stagger-3">
            ...
            <p className="text-sm text-muted-foreground">Share with the community and see if others agree your ending is better.</p>
          </div>
        </div>
      </section>
